...
# Test script voor de nieuwe subscription logica
echo "🧪 Test van nieuwe subscription selectie logica"
echo "=============================================="
...
if [[ "$1" == "single" ]]; then
    ...
    MOCK_SUBSCRIPTIONS=(
        $'Single Subscription\t12345678-1234-1234-1234-123456789012\ttrue'
    )
...
if [[ ! -z "$SUBSCRIPTION_ID" ]]; then
    ...
    if [ ${#MOCK_SUBSCRIPTIONS[@]} -eq 0 ]; then
        ...
        for i in "${!MOCK_SUBSCRIPTIONS[@]}"; do
            ...
            CURL_CMD="curl -s $SCRIPT_URL | bash -s -- \"$APP_NAME\" \"$GITHUB_ORG\" \"$GITHUB_REPO\" \"$GITHUB_REF\" \"$id\""
